...
| Estimated runtime memory | pending device measurement |

The INT4-quantized 4B model is viable on 8 GB RAM. FP16 weights or larger models are out of scope for v3.
...
1. **Model choice**: Qwen3-VL-4B-Instruct-MNN (INT4) is chosen because the 4B parameter count and INT4 quantization fit within the 6 GB runtime memory budget on 8 GB RAM devices. Larger models are deferred.
...
3. **Single APK, no root**: The deployment model is a standard user-space APK. Model weights are provisioned to `/sdcard/qwen3_vl_4b_mnn/` via ADB or in-app download; no system partition access is required.
