...
/**
 * Scriptable mock detector. Accepts an ordered sequence of TargetDetection outputs
 * (one per detect() call) so unit tests can drive all state-machine transitions.
...
class MockTargetDetector(
    ...
    companion object {
        ...
        /** No candidate ever (drives Searching). */
        fun noCandidateForever(): MockTargetDetector =
            MockTargetDetector(List(1000) { noFrame() })

        /** One frame with candidate then gone (noise; must not trigger Locking). */
        ...
        /**
         * Candidate stable for [count] consecutive frames.
         * count >= 2 enters Locking; count >= 2 + requiredStableFrames reaches Locked.
         */
        ...
        /** Candidate stable for [stableCount] frames then position jumps. */
        ...
        /** Stable candidate with one bad-quality frame at index [badAt], then recovers. */
        ...
        /** Persistent bad quality on every frame (quality keeps failing). */
