 ...
 *
 * Kotlin mirror of the S0 contract (`contracts/kotlin/StandardState.kt`,
 * `contracts/state_model.py`). This is the single source of truth for lifecycle
 * state on the Android Pad. The [wire] value is the canonical serialized form
 * that appears in every API payload, bundle manifest, and SQLite row — it MUST
 * stay byte-for-byte identical to the Python enum `value`. Display labels are
 * localized through the i18n seam ([com.giraffetechnology.qc.contracts.GiraffeLanguageSkill]),
 * never hard-coded; [i18nKey] gives the lookup key.
 *
 * This file is the in-app copy of the S0 deliverable so the Pad module compiles
 * standalone; the repo-root `contracts/` tree is reference-only and not built.
 */
