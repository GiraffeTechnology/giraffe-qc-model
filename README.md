...
**Readiness & completeness gate (PR 24, `src/qc_model/readiness/`):** makes `exam_ready` / `active` depend on **confirmed QC knowledge completeness** via a 10-check evaluator (sources reviewed, detection points confirmed, physical-measurement boundaries confirmed, rule-verification confirmed, visual rules reviewed, no unresolved questions, sample coverage, no unreviewed conflicts, no pending high-risk pseudo-defects, no pending critical defects). Insufficient sample coverage forces `on_trial` (never `active`). Only unresolved questions are **waivable** — per-item, requiring supervisor identity + justification, appended to an audit log; all other checks are non-waivable. **Behavioral change:** `exam_ready`/`active` transitions now consult this gate. Admin UI: `/admin/qc-model/training-packs/{training_pack_id}/readiness`.
