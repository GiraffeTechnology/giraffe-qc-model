...
| UI-visible state | Not in Stage 1 scope | Six test-only Chrome mock states validated, including fail-closed anomaly/unavailable and retry without duplication; Android evidence is supplemental | New Stage 2 acceptance surface; all states visibly marked `NON-PRODUCTION MOCK` and recorded zero inference calls | Re-run against physical Pad/Jetson integration and production navigation |
| Web control path | Not in Stage 1 UI scope | Administrator/Operator entry, bilingual Operator pages, tenant-scoped MySQL job creation, evidence gate, atomic checkpoint submission and persisted report are software-verified | Stage 2 now controls real qc-model records instead of only replaying display states | Repeat the role-boundary and persistence checks with the physical Jetson topology |
| Camera source | Fixture/simulated capture only | User amendment permits a Mac USB camera as simulated host capture; live hardware capture remains pending until the device is connected | Browser permission/device enumeration and Mac frame persistence are added; no Jetson camera stack is exercised | Re-test USB capture, reconnect, focus, low-light and occlusion on the physical Stage 3 device |
