...
2. `docs/STAGE3_AB_TESTING_SPEC.md` (the authoritative Group A / Group B
   definitions — **do not** reuse the repo's historical
   `scripts/run_capability_a_demo.py` / `run_capability_b_demo.py` naming or
   conclusions; see that spec's §3)
3. `docs/STAGE3_GROUP_B_REMOTE_ADAPTER.md` (Group B's remote adapter and
   restricted tunnel)

## Stage 3 A/B testing — requires the authorization gate to be open

Formal Stage 3 Group A / Group B testing may start **only** after a fresh
...
`scripts/jetson_stage3_run_group_a.py` / `run_group_b.py` and both refuse to
run (no report written) when the gate reports closed — there is no flag to
bypass this.
...
Physical camera capture, real Xavier NX inference (Group A and Group B),
role-boundary verification, per-stage timing, and defocus/low-light/occlusion
evidence — all per `docs/STAGE3_AB_TESTING_SPEC.md` §4's report schema and
§5's minimum acceptance criteria. Reports land in
